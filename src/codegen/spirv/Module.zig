...
/// An arena allocator, which is used to store things which have the same lifetime as this Module.
/// (like type data).
arena: *Allocator,
...
pub fn resolveTypeId(self: *Module, ty: Type) !IdRef {
    return self.typeByRef(try self.resolveType(ty));
}
...
pub fn emitType(self: *Module, ty: Type) !IdResultType {
    ...
    switch (ty.tag()) {
        ...
        .vector => try types.emit(self.gpa, .{.OpTypeVector = &.{
            .id_result = result_id,
            .component_type = self.typeByRef(ty.childType()).toRef(),
            .component_count = ty.payload(.vector).component_count,
        }}),
        .matrix => try types.emit(self.gpa, .{.OpTypeMatrix = &.{
            .id_result = result_id,
            .column_type = self.typeByRef(ty.childType()).toRef(),
        ...
        .image => {
            ...
            try types.emit(self.gpa, .{.OpTypeImage = &.{
                .id_result = result_id,
                .sampled_type = self.typeByRef(ty.childType()).toRef(),
                .dim = info.dim,
        ...
        .sampled_image => try types.emit(self.gpa, .{.OpTypeSampledImage = &.{
            .id_result = result_id,
            .image_type = self.typeByRef(ty.childType()).toRef(),
        }}),
        .array => {
            const info = ty.payload(.array);
            try types.emit(self.gpa, .{.OpTypeArray = &.{
                .id_result = result_id,
                .element_type = self.typeByRef(ty.childType()).toRef(),
        ...
        .@"struct" => {
            ...
            for (info.members) |member| {
                types.writeOperand(IdRef, self.typeByRef(member.ty).toRef());
            }
        ...
        .pointer => {
            ...
            try types.emit(self.gpa, .{.OpTypePointer = &.{
                ...
                .type = self.typeByRef(ty.childType()).toRef(),
            }});
            if (info.array_stride != 0) {
                try annotations.decorate(self.gpa, ref_id, .{.ArrayStride = .{.array_stride = info.array_stride}});
            }
        ...
        .function => {
            ...
            types.writeOperand(IdResult, result_id);
            types.writeOperand(IdRef, self.typeByRef(info.return_type).toRef());
            for (info.parameters) |parameter_type| {
                types.writeOperand(IdRef, self.typeByRef(parameter_type).toRef());
            }
        },
...
fn decorateStruct(self: *Module, target: IdRef, info: *Type.Payload.Struct) !void {
    const annotations = &self.sections.annotations;
...
pub fn typeByRef(self: Module, type_ref: Type.Ref) IdResultType {
