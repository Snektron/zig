...
pub const DeclGen = struct {
    ...
    fn airAssembly(self: *DeclGen, inst: Air.Inst.Index) !?IdRef {
        ...
        for (inputs) |input| {
            // TODO: It seems that inputs and outputs names are currently not yet encoded into the air, so just
            // use the constraint for now.
            // In the future these could be repurposed for specifying whether an input is a reference/label/etc.
            const constraint = std.mem.sliceTo(std.mem.sliceAsBytes(self.air.extra[input_extra_i..]), 0);
        ...
        for (outputs) |output| {
            ...
            output_extra_i += constraint.len / 4 + 1;

            const result = as.value_map.get(constraint) orelse return {
                return self.fail("invalid asm output '{s}'", .{constraint});
            };
