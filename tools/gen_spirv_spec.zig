...
pub fn main() !void {
    ...
    try render(bw.writer(), allocator, registry);
    try bw.flush();
}
...
fn render(writer: anytype, allocator: *Allocator, registry: g.Registry) !void {
    try writer.writeAll(
    ...
    switch (registry) {
        .core => |core_reg| {
            ...
            const extended_structs = try extendedStructs(allocator, core_reg.operand_kinds);
            try renderOpcodes(writer, allocator, core_reg.instructions, extended_structs);
            try renderOperandKinds(writer, allocator, core_reg.operand_kinds, extended_structs);
        },
        .extension => |ext_reg| {
            try writer.print(
            ...
            const extended_structs = try extendedStructs(allocator, ext_reg.operand_kinds);
            try renderOpcodes(writer, allocator, ext_reg.instructions, extended_structs);
            try renderOperandKinds(writer, allocator, ext_reg.operand_kinds, extended_structs);
        },
    }
}

fn parseVersion(maybe_version_str: ?[]const u8) !?std.builtin.Version {
    const version_str = maybe_version_str orelse return null;
    if (std.mem.eql(u8, version_str, "None")) {
        return null;
    }

    var it = std.mem.split(u8, version_str, ".");
    const major_str = it.next() orelse return error.InvalidVersion;
    const minor_str = it.next() orelse return error.InvalidVersion;

    return std.builtin.Version{
...
fn renderOpcodes(
    ...
    for (instructions_indices) |i| {
        ...
        if (try parseVersion(inst.version)) |v| {
            try writer.print(".version = .{{ .major = {}, .minor = {} }},\n", .{ v.major, v.minor });
        }

        if (try parseVersion(inst.lastVersion)) |v| {
            try writer.print(".last_version = .{{ .major = {}, .minor = {} }},\n", .{ v.major, v.minor });
        }

        if (inst.capabilities.len > 0) {
            try writer.writeAll(".capabilities = &.{");
            for (inst.capabilities) |cap, j| {
                if (j != 0) {
                    try writer.writeAll(", ");
                }

                // Fully qualified name required here to account for Capability aliases.
                try writer.print("Capability.{}", .{std.zig.fmtId(cap)});
            }
            try writer.writeAll("},\n");
        }

        if (inst.extensions.len > 0) {
            try writer.writeAll(".extensions = &.{");
            for (inst.extensions) |ext, j| {
                if (j != 0) {
                    try writer.writeAll(", ");
                }
    ...
    try writer.writeAll("pub const Instruction = union(Opcode) {\n");
    for (instructions_indices) |i| {
        const inst = instructions[i];
